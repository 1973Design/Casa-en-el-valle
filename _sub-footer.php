<div class="sub-footer">
    <div class="row">
        <div class="col-md-4 offset-md-2">
            <h5>Informatie</h5>
            <ul>
                <li><i class="fas fa-file-pdf"></i><a href="docs/huisregels-nl.pdf" target="_blank"> Huisregels</a></li>
                <li><i class="fas fa-file-pdf"></i><a href="docs/huisregels en huurvoorwaarde-website.pdf" target="_blank"> Algemenevoorwaarde</a></li>
            </ul>
                      
        </div>
        <div class="col-md-4 offset-md-2">
            <h5>Algemeen</h5>
            <ul>
                <li><i class="fas fa-link"></i><a href="faq-vakantiehuis.php"> FAQ</a></li>
                <li><i class="fas fa-link"></i><a href="privacystatement.php">Privacystatement</a></li>
                <li><i class="fas fa-link"></i><a href="handigelinks-vakantiehuis.php"> Handige links</a></li>
            </ul>
           
        </div>
    </div>
</div>